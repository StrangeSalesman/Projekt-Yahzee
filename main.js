...
class YahtzeeGame {
    ...
    finishRun() {
        ...
        if (player.remainingSection.upperSection.length === 0 && !player.state.bonusCalculated) {
            ...
            if(player.state.points.total >= 100) {
                alert(`Fine you beat me! Your final score is ${player.state.points.total}. The Pasword is "Würfelglück"`)
            }
            else {
                alert(`Well, thats not good enough! But im feeling kind today. Your final score is ${player.state.points.total}. The Password is "Pechsträhne"`)
            }
        }
